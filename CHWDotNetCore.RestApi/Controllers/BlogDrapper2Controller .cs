...
namespace CHWDotNetCore.RestApi.Controllers
{
 ...
 WHERE BlogId = @BlogId";
        ...
        public IActionResult PatchBlog(int id, BlogModel blog)
        {
                ...
                {
                    conditions.Add("[BlogTitle] = @BlogTitle");
                }

                if (!string.IsNullOrEmpty(blog.BlogAuthor))
                {
                    conditions.Add("[BlogAuthor] = @BlogAuthor");
                }
                if (!string.IsNullOrEmpty(blog.BlogContent))
                {
                    conditions.Add("[BlogContent] = @BlogContent");
                }

                string conditionsClause = string.Join(", ", conditions);

                string query = $@"UPDATE [dbo].[Tbl_Blog]
   SET {conditionsClause}
 WHERE BlogId = @BlogId";
